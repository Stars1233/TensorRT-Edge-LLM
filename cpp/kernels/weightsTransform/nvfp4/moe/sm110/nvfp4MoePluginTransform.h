...
inline cudaError_t launchNvfp4MoePluginFc1WeightTransform(uint8_t const* const* upSources,
    ...
    cudaStream_t stream)
{
    return launchNvfp4MoeFc1WeightTransformSourceBatch(
        upSources, gateSources, count, output, I, K, elementBytes, Nvfp4MoeFc1Layout::kInterleaved64, stream);
}

inline cudaError_t launchNvfp4MoePluginScaleTransform(uint8_t const* const* firstSources,
    uint8_t const* const* secondSources, int32_t count, int8_t* output, int32_t rows, int32_t Ksf, cudaStream_t stream)
{
    return launchNvfp4MoeScaleTransformSourceBatch(
        firstSources, secondSources, count, output, rows, Ksf, Nvfp4MoeFc1Layout::kInterleaved64, stream);
}
