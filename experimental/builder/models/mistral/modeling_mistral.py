...
"""Mistral checkpoint-direct graph aligned with Transformers."""
...
class MistralAttention(DecoderAttention):
    """Mistral attention extension point using configured sliding windows."""
    ...
    def __init__(self,
        ...
        super().__init__(ctx,
                         prefix,
                         layer_index,
                         sliding_window_size=ctx.cfg.sliding_window_size)


class MistralDecoderLayer(DecoderLayer):
    """Mistral decoder layer composed from shared primitive modules."""

    attention_class = MistralAttention


class MistralModel(DecoderModel):
    """Mistral decoder stack and family extension point."""

    layer_class = MistralDecoderLayer


class MistralForCausalLM(NetworkModule):
    """Mistral engine component and its explicit runtime I/O contract."""

    def __init__(self, ctx: BuildContext) -> None:
        super().__init__(ctx)
        self.model = MistralModel(ctx)
