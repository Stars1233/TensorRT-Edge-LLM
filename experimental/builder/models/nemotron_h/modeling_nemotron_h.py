...
"""Nemotron-H checkpoint-direct graph."""
...
LOGGER = logging.getLogger("builder.nemotron_h")


class NemotronHAttention(DecoderAttention):
    """Nemotron-H attention extension point."""
...
class NemotronHMamba2Mixer(Module):
    ...
    def forward(self, hidden_states, conv_state, recurrent_state,
        ...
        dt = F.slice_last_dim(projected, d_inner + mamba.conv_dim,
                              mamba.num_heads, 3)
        dt = dt.maximum(np.float16(-50.0)).minimum(np.float16(50.0))
...
class NemotronHMLP(Module):
    """Nemotron-H non-gated feed-forward layer."""
...
class NemotronHMoE(Module):
    """Nemotron-H sigmoid-grouped NVFP4 MoE plus its shared expert."""
    ...
    def __init__(self, ctx, prefix: str) -> None:
        ...
        self.shared_experts = NemotronHMLP(ctx, self.key("shared_experts"))
...
class NemotronHBlock(Module):
    """One Nemotron-H recurrent, attention, dense, or MoE block."""

    def __init__(self, ctx, prefix: str, layer_type: str) -> None:
        super().__init__(ctx, prefix)
        self.layer_type = layer_type
        self.input_norm = RMSNorm(ctx, self.key("norm"), ctx.cfg.rms_norm_eps)
        if layer_type == config.LAYER_MAMBA:
            self.mixer = NemotronHMamba2Mixer(ctx, self.key("mixer"))
        elif layer_type == config.LAYER_ATTN:
            self.mixer = NemotronHAttention(ctx, self.key("mixer"))
        elif layer_type == config.LAYER_MLP:
            self.mixer = NemotronHMLP(ctx, self.key("mixer"))
        elif layer_type == config.LAYER_MOE:
            self.mixer = NemotronHMoE(ctx, self.key("mixer"))
        else:
            raise ValueError(
                f"unsupported Nemotron-H layer type {layer_type!r}")
...
class NemotronHForCausalLM(NetworkModule):
    """Top-level Nemotron-H model with compact cache indexing."""

    def __init__(self, ctx) -> None:
        super().__init__(ctx)
        self.qwen35 = False
        self.layers = [
            NemotronHBlock(ctx, f"backbone.layers.{index}", layer_type)
