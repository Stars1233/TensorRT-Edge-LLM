...
"""Qwen2-VL tokenizer artifacts."""

from typing import Any, Dict

QWEN2_IMAGE_PLACEHOLDER = "<|vision_start|><|image_pad|><|vision_end|>"
QWEN2_VIDEO_PLACEHOLDER = "<|vision_start|><|video_pad|><|vision_end|>"
...
def patch_chat_template(template: Dict[str, Any],
    ...
    content_types.setdefault("image",
                             {}).update({"format": QWEN2_IMAGE_PLACEHOLDER})
    if root_config.get("video_token_id") is not None:
        content_types.setdefault("video", {}).update(
            {"format": QWEN2_VIDEO_PLACEHOLDER})
