...
def write_talker_embeddings(weights, root: Dict[str, Any],
                            output_dir: str) -> None:
    """Write Qwen3-TTS talker-owned embedding tensors."""
    text_key = "talker.model.text_embedding.weight"
    if weights.store.has(text_key):
        save_safetensors(
            os.path.join(output_dir, "text_embedding.safetensors"),
            {"text_embedding": weights.store.get_f16(text_key)})
...
def write_code_predictor_embeddings(weights, root: Dict[str, Any],
                                    output_dir: str) -> None:
    """Write Qwen3-TTS code-predictor embedding and head tensors."""
    talker = _talker(root)
