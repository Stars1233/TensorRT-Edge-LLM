...
"""Alpamayo Qwen3-VL vision model."""
...
class AlpamayoVisualMerger(VisionPatchMerger):
    """Alpamayo patch-merger extension point."""


class AlpamayoVisualBlock(VisionTransformerBlock):
    """Alpamayo visual-block extension point."""


class AlpamayoVisualEncoder(NetworkModule):
    """Alpamayo-owned Qwen3-VL vision transformer."""
    ...
    def __init__(self, ctx, bundle) -> None:
        ...
        self.blocks = [
            AlpamayoVisualBlock(ctx, prefix, self.hidden_size, self.num_heads,
                                self.hidden_act)
        ...
        self.deepstack_mergers = {
            layer:
            AlpamayoVisualMerger(
                ctx, f"{self.patch_embed.visual_prefix}."
                f"deepstack_merger_list.{index}", self.hidden_size,
                self.merge_unit, True)
            for index, layer in enumerate(
                self.visual.get("deepstack_visual_indexes", []))
        }
        self.final_merger = AlpamayoVisualMerger(
    ...
    def forward(self, pixels, rotary, cu_seqlens, max_seqlen,
                fast_pos_embed_idx, fast_pos_embed_weight):
        hidden = self.patch_embed(pixels)
        hidden = self.fast_position(hidden, fast_pos_embed_idx,
                                    fast_pos_embed_weight)
        deepstack = []
        for index, block in enumerate(self.blocks):
            hidden = block(hidden, rotary, cu_seqlens, max_seqlen)
            if index in self.deepstack_mergers:
                deepstack.append(self.deepstack_mergers[index](hidden))

        merged = self.final_merger(hidden)
        merged.name = "output"
        for index, feature in enumerate(deepstack):
            feature.name = f"deepstack_features_{index}"
