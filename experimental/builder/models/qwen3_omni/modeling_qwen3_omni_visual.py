...
class Qwen3OmniVisualMerger(VisionPatchMerger):
    """Qwen3-Omni merger using the provider's ``ln_q``/``mlp`` keys."""
...
class Qwen3OmniVisualBlock(VisionTransformerBlock):
    """Qwen3-Omni visual-block extension point."""


class Qwen3OmniVisualEncoder(NetworkModule):
    ...
    def __init__(self, ctx, bundle) -> None:
        ...
        self.blocks = [
            Qwen3OmniVisualBlock(ctx, prefix, self.hidden_size, self.num_heads,
                                 self.hidden_act)
        ...
        self.final_merger = Qwen3OmniVisualMerger(
