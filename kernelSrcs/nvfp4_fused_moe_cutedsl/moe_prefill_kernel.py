...
"""MoEPrefillKernel — queue-driven routed NVFP4 MoE kernel for SM120/SM121.

Ported from the b12x kernel library (originally ``MoEDynamicKernel``).

This is the first prefill fused control-plane kernel derived from the decode
implementation. It keeps the proven FC1 / SiLU / quant / FC2 / scatter compute
body, but replaces the resident-grid route/pack -> compute barrier with a
...
This is intentionally conservative:
  ...
  - still the decode per-slice microkernel, now executed sequentially for a
    small grouped slice task
  - still one initial resident-grid barrier after init

What changes relative to the decode path
...
class MoEPrefillKernel:
    """Queue-driven first-pass prefill MoE kernel."""
    ...
    def _compute_total_smem(self) -> int:
        """Estimate total shared memory bytes for the current staged layouts.

        Mirrors b12x's _shared_storage_size_bytes: accumulates the A, B,
        SFA, SFB and epilogue staged layout sizes, adds the gated B_up/SFB_up
        duplication for swiglu, and accounts for alignment padding between
        buffers.  Used by _setup_attributes to check whether the chosen
        ab_stage fits in the device smem budget.
        """
        ...
        buffers = [
            _cute.size_in_bytes(self.a_dtype,  self.a_smem_layout_staged),
            _cute.size_in_bytes(self.b_dtype,  self.b_smem_layout_staged),
            _cute.size_in_bytes(self.sf_dtype, self.sfa_smem_layout_staged),
            _cute.size_in_bytes(self.sf_dtype, self.sfb_smem_layout_staged),
            _cute.size_in_bytes(self.io_dtype, self.epi_smem_layout_staged),
        ]
        if self.is_gated:
            # swiglu requires extra sB_up and sSFB_up buffers
            buffers.insert(2, _cute.size_in_bytes(self.b_dtype,  self.b_smem_layout_staged))
            buffers.insert(5, _cute.size_in_bytes(self.sf_dtype, self.sfb_smem_layout_staged))

        # Control/pipeline overhead (ctrl Int32×8, pipeline arrays Int64×ab_stage×2,
        # plus up_pipeline_array for gated, and two tile-index arrays).
        pipeline_count = 3 if self.is_gated else 2
        overhead = (
            8 * 4                                      # ctrl
            + pipeline_count * self.ab_stage * 2 * 8  # pipeline arrays (Int64)
            + 2 * self.tile_shape_mnk[0] * 4          # tile_idx arrays (Int32)
        )

        # Sum buffers with buffer_align_bytes padding between them
        total = overhead
        for i, sz in enumerate(buffers):
            if i > 0:
                total = _align_up(total, self.buffer_align_bytes)
            total += sz
        return total
    ...
    def _setup_attributes(self, hidden_size: int):
        ...
        while True:
            ...
            if self._compute_total_smem() <= self.smem_capacity or self.ab_stage <= 1:
                break
            # Smem overflows (e.g. swiglu at large H): shrink ab_stage and keep
            # it a divisor of k_tile_cnt to preserve pipeline-phase alignment.
            self.ab_stage -= 1
            while self.ab_stage > 1 and k_tile_cnt % self.ab_stage != 0:
                self.ab_stage -= 1
    ...
    def __call__(
        ...
        # Prefill never materializes the intermediate C tensor. Preserve the
        ...
        # gate_tile_cnt now means "number of scheduling slices per expert".
        # Gated: 2*I_tp B-tiles, paired into I_tp/tile_N slices → divide by 2.
        # Non-gated: I_tp B-tiles, one per slice → no division.
    ...
    def kernel(
        ...
        # Single tiled view over w13.
        #   Gated: [2*I_tp, K, E] packed as [up | gate] across N.
        #     Up tiles: N=0..gate_tile_cnt-1
        #     Gate tiles: N=gate_tile_cnt..2*gate_tile_cnt-1
        #   Non-gated: [I_tp, K, E] — single FC1 B view, N=0..intermediate_tile_cnt-1
        ...
        # gate_tile_cnt = "slices per expert":
        #   Gated: w13 has 2*I_tp/tile_N N-tiles; paired into I_tp/tile_N slices.
        #   W13 is packed as [up | gate] across the concatenated N dimension:
        #   up tiles at N=0..gate_tile_cnt-1, gate tiles at
        #   N=gate_tile_cnt..2*gate_tile_cnt-1.
        #   Non-gated: w1 has I_tp/tile_N N-tiles, one per slice.
        ...
        while consumer_live > Int32(0):
            ...
            if has_task == Int32(0):
                ...
                while slice_idx < task_slice_count_val:
                    ...
                    if cutlass.const_expr(self.is_gated):
                        # Gate and up share the A/SFA staging buffers. Drain
                        # gate consumers before the DMA warp starts refilling
                        # those stages for the up pass.
                ...
                while slice_idx < task_slice_count_val:
                    ...
                    if cutlass.const_expr(self.is_gated):
                        ...
                        # N dimension: up at 0..gate_tile_cnt-1, gate at
                        # gate_tile_cnt..2*gate_tile_cnt-1.
                    ...
                    tBgB_w13_gate_nk = tBgB_w13[
                        (None, fc1_b_tile_idx, None, task_expert_idx)
                    ]
                    ...
                    if cutlass.const_expr(self.is_gated):
                        # Gate and up share the A/SFA staging buffers. Wait
                        # for MMA warps to drain the gate pass before refilling.
                        self.pass_sync_barrier.arrive_and_wait()
