...
"""MoEDecodeKernel — decode-scheduled routed NVFP4 MoE kernel for SM120/SM121 (Blackwell).

Ported from the b12x kernel library (originally ``MoEStaticKernel``).

This is the current decode control-plane fusion step: keep the proven FC1/FC2
...
The compute half supports configurable activation via the `activation` parameter:
  Gated (activation="swiglu"):
    FC1:     A x gate^T, A x up^T   (paired FP4 block-scaled GEMMs)
    Act:     SiLU(gate) * up         (fused SwiGLU activation)
  Non-gated (activation="identity", "silu", "gelu", "relu"):
    FC1:     A x W1^T               (single FP4 block-scaled GEMM)
    Act:     f(FC1_out)             (element-wise activation)
  Common:
...
What changes relative to the old split path:
  ...
  The decode kernel builds those GPU-side before entering the same grouped
  compute schedule. That is why this file owns the resident-grid barriers and
  the route/pack bookkeeping itself.
...
Work decomposition
  ...
  Compute:
    The compact decode work loop assigns (m_tile, intermediate_slice, expert).
...
Why the barriers exist
  ...
  been written. The decode kernel therefore uses a resident-grid barrier between
...
Design boundary
  The decode kernel is the compact backend. It keeps route/pack and compute in
  one resident launch for small routed working sets, and relies on the
  resident-grid barrier between those phases instead of overlapping them.
  Large routed workloads dispatch to the prefill backend instead.
"""
...
# DEBUG: opt-in smem dump. When the env var is set at import time the MMA
# warp writes sSFB[0..15] (stage 0) into scatter_output[0, 0..15] as bf16
# byte values after the first FC1 TMA load. Used to verify whether the TMA
# descriptor delivered the right SF bytes. Zero cost when the env var is
# unset -- `cutlass.const_expr` eliminates the branch at JIT trace time.
# The caller must route so token 0's token_final_scales=0 to avoid FC2
# scatter clobbering the dump.
...
def _ld_shared_u8(addr, *, loc=None, ip=None):
    """Load a single unsigned byte from shared memory at u32 addr."""
    return Int32(
        llvm.inline_asm(
            T.i32(),
...
class MoEDecodeKernel:
    """Compact decode MoE kernel for small routed working sets."""
    ...
    def __init__(
        ...
        *,
        ...
        io_dtype: type = cutlass.BFloat16,
    ):
        self._dense_cls = DenseGemmKernel
        self.acc_dtype = cutlass.Float32
        self.sf_vec_size = sf_vec_size
        self.activation = activation
        self.is_gated = activation in ("swiglu", "geglu")
        self.input_scales_are_reciprocal = input_scales_are_reciprocal
        self.fast_math = fast_math
        # Dtype for the input activation, epilogue smem, and scatter output.
        # Both fp16 and bf16 route through f32 inside the MMA, so io_dtype
        # only matters at the input quantization and output scatter boundaries.
        ...
        # ``output_tile_count_n`` is derived at runtime inside ``kernel()`` from
        # ``cute.size(gB_down, mode=[2])`` (see local ``output_tile_cnt``). No
        # compile-time copy is held here, so the kernel accepts arbitrary N
        # that is a multiple of ``mma_tiler_mn[1]``.
    ...
    def _shared_storage_size_bytes(
        ...
        num_pipelines = 3 if self.is_gated else 2
        offset = (
            3 * 4
            + num_pipelines * (self.ab_stage * 2 * 8)
        ...
        if cutlass.const_expr(self.is_gated):
            buffers.append(cute.size_in_bytes(self.b_dtype, b_smem_staged))
        buffers.append(cute.size_in_bytes(self.sf_dtype, sfa_smem_staged))
        buffers.append(cute.size_in_bytes(self.sf_dtype, sfb_smem_staged))
        if cutlass.const_expr(self.is_gated):
            buffers.append(cute.size_in_bytes(self.sf_dtype, sfb_smem_staged))
        buffers.append(cute.size_in_bytes(self.io_dtype, epi_smem_staged))
    ...
    def __call__(
        ...
        b_w13: cute.Tensor,  # [2*I_tp, K, E] gated or [I_tp, K, E] non-gated
        sfb_w13_ptr: cute.Pointer,  # scale factors for w13 / w1
        ...
        # Compact decode always scatters into token-major row-major output.
        ...
        # Compact decode schedules over (m_tile, intermediate_slice, local_expert_idx).
    ...
    def kernel(
        ...
        if cutlass.const_expr(self.is_gated):
            ...
            class Storage:
                ...
                pipeline_array: cute.struct.MemRange[
                    cutlass.Int64, self.ab_stage * 2
                ]
                up_pipeline_array: cute.struct.MemRange[
                    cutlass.Int64, self.ab_stage * 2
                ]
                ...
                sSFA: cute.struct.Align[
                    cute.struct.MemRange[
                        self.sf_dtype, cute.cosize(sfa_smem_staged)
                    ],
                    self.buffer_align_bytes,
                ]
                sSFB: cute.struct.Align[
                    cute.struct.MemRange[
                        self.sf_dtype, cute.cosize(sfb_smem_staged)
                    ],
                    self.buffer_align_bytes,
                ]
                ...
                sC: cute.struct.Align[
                    cute.struct.MemRange[
                        self.io_dtype, cute.cosize(epi_smem_staged)
                    ],
                    self.buffer_align_bytes,
                ]
        ...
        while j < scatter_total:
            scatter_output[j // cols, j % cols] = self.io_dtype(0.0)
            j += flat_stride
        cute.arch.sync_threads()
        ...
        while pair_idx < total_pairs:
            ...
            # `slot` = compact per-active-expert index (also called local_expert_id
            # elsewhere in the file). Assigned by the 3-state CAS protocol below.
            slot = Int32(0)
            row = Int32(0)
            if is_cta_leader > Int32(0):
                prior_slot = _atomic_cas_global_i32(
                    get_ptr_as_int64(global_to_local_expert, expert_id),
                    Int32(-1),
                    Int32(-2),
                )
                if prior_slot == Int32(-1):
                    slot = atomic_add_global_i32(
                        get_ptr_as_int64(active_expert_count, Int32(0)),
                        Int32(1),
                    )
                    weight_expert_ids[slot] = expert_id
                    _st_global_release_i32(
                        get_ptr_as_int64(global_to_local_expert, expert_id),
                        slot,
                    )
                else:
                    if prior_slot == Int32(-2):
            ...
            while sf_idx < sf_blocks_per_row:
                ...
                output_offset = (
                    slot * max_rows * output_bytes_per_row
        ...
        # W13 TMA partition (gate+up concatenated when gated, single w1 otherwise)
        ...
        if warp_idx < self.num_mma_warps:
            ...
            while is_valid_tile:
                ...
                tRS_rD_out = cute.make_rmem_tensor(
                    tRS_rD_layout.shape, self.io_dtype
                )
                ...
                if cutlass.const_expr(self.is_gated):
                    ...
                    # consumers before the DMA warp starts refilling those
                    # stages for the up pass.
                            ...
                            for mma_m_in_epi in cutlass.range_constexpr(MmaMPerEpiM):
                                ...
                                for elem_idx in cutlass.range_constexpr(
                                    ...
                                    if cutlass.const_expr(self.is_gated):
                                        ...
                                        v = alpha_value * gate_slice[elem_idx]
                                        sigmoid_v = cute.arch.rcp_approx(
                                            cutlass.Float32(1.0)
                                            + cute.math.exp(
                                                -v, fastmath=self.fast_math
                                            ),
                                        )
                                        tRS_rD_slice[elem_idx] = v * sigmoid_v
                                        ...
                                        tRS_rD_slice[elem_idx] = relu_v * relu_v
                ...
                for output_tile_idx in range(0, output_tile_cnt, 1, unroll=4):  # type: ignore[call-overload]
                    ...
                    for epi_m in cutlass.range_constexpr(epi_rest_m):
                        ...
                        while pair_idx < warp_epi_rows * Int32(32):
                            ...
                            if cutlass.const_expr(self.io_dtype == cutlass.Float16):
                                scatter_add_f16x2(
                                    get_ptr_as_int64(
        ...
        # DMA WARP (warp 4)
            ...
            while is_valid_tile:
                ...
                if cutlass.const_expr(self.is_gated):
                    # W13 is laid out as [up, gate] across the concatenated
                    # N dimension.
                    tBgB_w13_up_nk = tBgB_w13[
                        (None, intermediate_slice, None, weight_expert_idx)
                    ]
                    sfb_up_tile_coord = (
                        intermediate_slice // self.sfb_tiles_per_block
                    )
                    tBgSFB_w13_up_nk = tBgSFB_w13[
                        (None, sfb_up_tile_coord, None, weight_expert_idx)
                    ]
                    fc1_b_tile_idx = intermediate_slice + gate_tile_cnt
                else:
                    fc1_b_tile_idx = intermediate_slice
                tBgB_w13_gate_nk = tBgB_w13[
                    (None, fc1_b_tile_idx, None, weight_expert_idx)
                ]
                sfb_gate_tile_coord = (
                    fc1_b_tile_idx // self.sfb_tiles_per_block
                )
                tBgSFB_w13_gate_nk = tBgSFB_w13[
                    (None, sfb_gate_tile_coord, None, weight_expert_idx)
                ]

                # ---- FC1 gate pass (or single FC1 pass when non-gated) ----
                ...
                if cutlass.const_expr(self.is_gated):
                    # Gate and up share the A/SFA staging buffers. Wait for
                    # MMA warps to drain the gate pass before refilling.
                    self.pass_sync_barrier.arrive_and_wait()
